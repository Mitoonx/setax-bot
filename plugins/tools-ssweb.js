...
let handler = async (m, { conn, command, args, text }) => {
    ...
    if (!text) return m.reply(`*[✳️]* INGRESE EL URL DE UNA PAGINA WEB.\n*[✳️]* *RESPUESTA :* IMAGEN DE LA URL ENVIADA.`)
    m.react(rwait) 
   ...
   // let ss = await fg.ssweb(url, /f$/i.test(command), args[1])
    conn.sendFile(m.chat, ss, 'ssweb.png', `*[✳️]* *RESULTADO SUCCESSFUL*`, m) 
   m.react(done) 
}
